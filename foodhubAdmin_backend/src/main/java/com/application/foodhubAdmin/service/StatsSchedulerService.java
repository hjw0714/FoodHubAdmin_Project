...
public class StatsSchedulerService {
    ...
    public void runStats() {
        LocalDate yesterday = LocalDate.now().minusDays(1);
            userMsService.insertUserStatsJoin(yesterday); // 유저 가입수1
            userMsService.insertUserStatsDelete(yesterday); // 유저 탈퇴수2
            userMsService.insertUserStatsTotal(yesterday); // 유저 총수3
            postMsService.insertPostStatsTotal(yesterday); // 게시글 총 작성수4
            postMsService.insertPostCategoryStatsTotal(yesterday); // 게시글 카테고리별 총 작성수 5~12
            commentsService.insertCommentsStatsTotal(yesterday); // 댓글 총 작성수13
